...
class Home extends React.Component {
	...
	dosomething_onClick = (e) => {
		var { onExecuteDoSomething } = this.props

		onExecuteDoSomething()
	}
		...
		return (
			<div className={classes.root}>
				<Paper elevation={5} className={classes.paper}>
					<Button onClick={this.dosomething_onClick} className={classes.export} size="small">
						DO SOMETHING
					</Button>
					<List>
						<PluginItem
							ref={this.textureItemRef}
							index="0"
							title="Textures"
							desc="export textures"
							icon="texture"
						/>
						<PluginItem ref={this.masksItemRef} index="1" title="Masks" desc="export masks" icon="layers" />
						<PluginItem ref={this.infoItemRef} index="2" title="Info" desc="export info json" icon="info" />
						<PluginItem
							ref={this.hierarchicalItemRef}
							index="3"
							title="Hierarchical"
							desc="hierarchical display list or flatten"
							icon="list"
						/>
						<PluginItem
							ref={this.inspectVisibleItemRef}
							index="4"
							title="Visibility"
							desc="inspect only visible layers"
							icon="visibility"
						/>
						<PluginItem
							ref={this.namesItemRef}
							index="5"
							title="Names"
							desc="maningful names for assets"
							icon="build"
						/>
						<BrowseItem innerRef={this.browseItemRef} />
					</List>
					<Button onClick={this.export_onClick} className={classes.export} size="small">
						Export
					</Button>
				</Paper>
			</div>
