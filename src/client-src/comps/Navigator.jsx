...
class Navigator extends React.Component {
	...
	handleChange = (event, value) => {
		...
		if (onNavigateChange) onNavigateChange(value, data[value].title)
	}
	...
	tabContentFactory(data, index, active, classes) {
		...
		return (
			<Hideable key={index} visible={active}>
				{comp}
			</Hideable>
		)
	}
		...
		return (
			<div className={classes.root}>
				<div className="app__header">
					<img className="app__header-icon" src={app_icon} />
					<span className="app__header-title">
						Velvet<span className="app__header-title-dash">-</span>DPS Sketch Connector
						<span className="app__version-label">{process.env.RELEASE_VERSION}</span>
					</span>
				</div>
				<AppBar position="static">
					<Tabs fullWidth value={activeTabIndex} onChange={this.handleChange}>
